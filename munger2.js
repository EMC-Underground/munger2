...
/***************************************************************************************************************/
var thingToCount = "SEV", // Put the install base item that you want to count here
...
function processThingToCount(GDUNlist, callback) {
	...
	async.forEachSeries(thingToCountList, function(thingToCountItem, callback) {
		...
		processGDUN(thingToCountItem, GDUNlist, function(err) {
			if (err) {
				console.log("processThingToCount - in processGDUN code");
		...
		async.series([
			...
			function(callback) {
				getIBdata(thingToCountItem, gdun, function(err, insight) {
					if (err) {
						console.log('processGDUN - Error getting SR data for GDUN=' + gdun + ': ' + err);
...
function getIBdata(thingToCountItem, gdun, callback) {
	...
	//ecs.headObject(params, function(err, data) {
		//if (err + err.code === 'NotFound') {
			//console.log (err + err.code + 'in headObject');
		//} else {
		ecs.getObject(params, function(err, data) {
			if (err) {
				callback(err);
				...
				try {
					...
					callback('unexpected SR JSON format in ' + key);
				}
			}
		});
	//}});
